# =============================================================================
# LXC Base Template - Terraform Variables
# =============================================================================
#
...
variable "lxc_description" {
  ...
  type        = string
  default     = "Grafana Observability Platform - Managed by OpenTofu"
}

variable "lxc_tags" {
  description = "Tags for container organization in Proxmox"
  type        = list(string)
  default     = ["lxc", "grafana", "monitoring", "tofu-managed"]
}
...
# -----------------------------------------------------------------------------
# Grafana Data Bind Mount
# -----------------------------------------------------------------------------

variable "lxc_grafana_data_mount_volume" {
  description = "Host path for Grafana data directory (SQLite DB, plugins, sessions)"
  type        = string
  default     = "/rpool/data/grafana"
}

variable "lxc_grafana_data_mount_path" {
  description = "Container path for Grafana data"
  type        = string
  default     = "/var/lib/grafana"
}

# -----------------------------------------------------------------------------
# Service User UID/GID (Unprivileged Container Support)
# -----------------------------------------------------------------------------

variable "service_user_uid" {
  description = "UID for grafana service user (maps to 100900 on host in unprivileged mode)"
  type        = number
  default     = 900

  validation {
    condition     = var.service_user_uid >= 100 && var.service_user_uid <= 65533
    error_message = "Service user UID must be between 100 and 65533."
  }
}

variable "service_user_gid" {
  description = "GID for grafana service group"
  type        = number
  default     = 900

  validation {
    condition     = var.service_user_gid >= 100 && var.service_user_gid <= 65533
    error_message = "Service user GID must be between 100 and 65533."
  }
}
