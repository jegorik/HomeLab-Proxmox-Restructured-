# =============================================================================
# LXC Grafana - Main Terraform Configuration
# =============================================================================
# Deploys Grafana observability platform in LXC container on Proxmox
#
...
resource "terraform_data" "fix_bind_mount_permissions" {
  ...
  triggers_replace = [
    ...
    var.lxc_grafana_data_mount_volume,
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # Fix /var/lib/grafana mount
      "/tmp/fix_bind_mount_permissions.sh '${var.lxc_grafana_data_mount_volume}' '${var.service_user_uid}' '${var.service_user_gid}'",
...
resource "proxmox_virtual_environment_container" "grafana" {
  ...
  # Bind Mount Configuration for Grafana
  # ---------------------------------------------
  # Grafana stores all data in /var/lib/grafana (SQLite DB, plugins, sessions)

  mount_point {
    volume = var.lxc_grafana_data_mount_volume
    path   = var.lxc_grafana_data_mount_path
  }
