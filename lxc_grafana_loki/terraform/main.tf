# =============================================================================
# LXC Grafana Loki - Main Terraform Configuration
# =============================================================================
# Deploys Grafana Loki in an LXC container on Proxmox
#
...
resource "terraform_data" "fix_bind_mount_permissions" {
  ...
  triggers_replace = [
    ...
    var.lxc_grafana_loki_chunks_mount_volume,
    var.lxc_grafana_loki_rules_mount_volume,
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "# Create and fix permissions for Grafana Loki data directory",
      "/tmp/fix_bind_mount_permissions.sh '${var.lxc_grafana_loki_chunks_mount_volume}' '${var.service_user_uid}' '${var.service_user_gid}'",
      "/tmp/fix_bind_mount_permissions.sh '${var.lxc_grafana_loki_rules_mount_volume}' '${var.service_user_uid}' '${var.service_user_gid}'",
...
resource "proxmox_virtual_environment_container" "loki" {
  ...
  # Bind Mount for Grafana Loki Data Persistence
  mount_point {
    volume = var.lxc_grafana_loki_chunks_mount_volume
    path   = var.lxc_grafana_loki_chunks_mount_path
  }

  # Bind Mount for Grafana Loki Rules Persistence
  mount_point {
    volume = var.lxc_grafana_loki_rules_mount_volume
    path   = var.lxc_grafana_loki_rules_mount_path
  }
...
resource "terraform_data" "ansible_user_setup" {
  ...
  triggers_replace = [
    proxmox_virtual_environment_container.loki.id,
  ]

  # Ensure container is fully created before user setup
  depends_on = [proxmox_virtual_environment_container.loki]
