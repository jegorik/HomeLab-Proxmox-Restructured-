# =============================================================================
# LXC Grafana Loki - Terraform Variables
# =============================================================================
# Grafana Loki Log Aggregation Service Container Configuration
...
variable "lxc_description" {
  ...
  type        = string
  default     = "Grafana Loki Log Aggregation Service - Managed by OpenTofu"
}

variable "lxc_tags" {
  description = "Tags for container organization in Proxmox"
  type        = list(string)
  default     = ["lxc", "grafana_loki", "logging", "tofu-managed"]
}
...
variable "lxc_memory" {
  ...
  validation {
    condition     = var.lxc_memory >= 256
    error_message = "Minimum memory is 256 MB."
  }
}
...
# -----------------------------------------------------------------------------
# Grafana Loki-Specific Configuration
# -----------------------------------------------------------------------------

variable "lxc_grafana_loki_chunks_mount_volume" {
  description = "Host path for Grafana Loki chunks directory (bind mount)"
  type        = string
  default     = "/rpool/data/grafana_loki/chunks"
}

variable "lxc_grafana_loki_chunks_mount_path" {
  description = "Container path for Grafana Loki chunks"
  type        = string
  default     = "/var/lib/loki/chunks"
}

variable "lxc_grafana_loki_rules_mount_volume" {
  description = "Host path for Grafana Loki rules directory (bind mount)"
  type        = string
  default     = "/rpool/data/grafana_loki/rules"
}

variable "lxc_grafana_loki_rules_mount_path" {
  description = "Container path for Grafana Loki rules"
  type        = string
  default     = "/var/lib/loki/rules"
}


variable "service_user_uid" {
  description = "UID for Grafana Loki service user (maps to 100900 on host for unprivileged containers)"
  type        = number
  default     = 900

  validation {
    condition     = var.service_user_uid >= 100 && var.service_user_uid <= 65535
    error_message = "Service user UID must be between 100 and 65535."
  }
}

variable "service_user_gid" {
  description = "GID for Grafana Loki service group (maps to 100900 on host for unprivileged containers)"
  type        = number
  default     = 900

  validation {
    condition     = var.service_user_gid >= 100 && var.service_user_gid <= 65535
