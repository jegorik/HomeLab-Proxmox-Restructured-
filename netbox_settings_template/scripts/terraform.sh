#!/usr/bin/env bash
# =============================================================================
# Terraform/OpenTofu Functions for NetBox Settings Template
# =============================================================================
# Handles all Terraform/OpenTofu operations
...
terraform_init() {
    ...
    if [[ -f "s3.backend.config" ]]; then
        ...
        log_warning "s3.backend.config not found - using LOCAL state"
        log_warning "Create s3.backend.config from template for remote state"
    fi
...
terraform_apply() {
    ...
    log_warning "This will apply NetBox configuration changes"
...
# -----------------------------------------------------------------------------
terraform_destroy() {
    log_header "Destroying NetBox Configuration"
    
    local iac_tool
    iac_tool=$(get_iac_tool) || return 1
    cd "${TERRAFORM_DIR}" || return 1

    log_warning "This will DESTROY all managed NetBox resources!"
