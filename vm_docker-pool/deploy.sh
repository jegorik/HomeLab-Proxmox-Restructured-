#!/usr/bin/env bash
# =============================================================================
# VM Docker Pool - Deployment Script
# =============================================================================
...
deploy_full() {
    ...
    log_info "Waiting for VM to fully boot (cloud-init may take a few minutes)..."
...
deploy_destroy() {
    ...
    log_info "Note: Portainer data in /rpool/datastore/portainer is preserved"
}
...
deploy_status() {
    ...
    if [[ -n "${vm_ip}" ]]; then
        ...
        if nc -z -w 2 "${vm_ip}" 9443 2>/dev/null; then
            log_success "Portainer is reachable at ${vm_ip}:9443"
        else
            log_warning "Portainer is not reachable at ${vm_ip}:9443"
        fi
    fi
}
...
show_menu() {
    echo ""
    echo -e "${BOLD}${CYAN}VM Docker Pool - Deployment Menu${NC}"
    echo "======================================"
...
main() {
    log_info "Starting VM Docker Pool deployment"
