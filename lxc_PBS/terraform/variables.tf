...
variable "lxc_description" {
  ...
  type        = string
  default     = "Proxmox Backup Server - Managed by OpenTofu"
}

variable "lxc_tags" {
  description = "Tags for container organization in Proxmox"
  type        = list(string)
  default     = ["lxc", "pbs", "backup", "tofu-managed"]
}
...
variable "lxc_pbs_config_mount_volume" {
  ...
  type        = string
  default     = "/rpool/data/pbs-config"
}

variable "lxc_pbs_config_mount_path" {
  description = "Container path for PBS configuration"
  type        = string
  default     = "/etc/proxmox-backup"
}

variable "lxc_pbs_datastore_mount_volume" {
  description = "Host path for PBS datastore (backups)"
  type        = string
  default     = "/rpool/data/pbs-backups"
}

variable "lxc_pbs_datastore_mount_path" {
  description = "Container path for PBS datastore"
  type        = string
  default     = "/mnt/datastore"
}

variable "lxc_pbs_s3_cache_mount_volume" {
  description = "Host path for PBS S3 cache"
  type        = string
  default     = "/rpool/data/pbs-s3-cache"
}

variable "lxc_pbs_s3_cache_mount_path" {
  description = "Container path for PBS S3 cache"
  type        = string
  default     = "/var/cache/proxmox-backup/s3"
}
