# =============================================================================
# LXC Semaphore UI - Terraform Variables
# =============================================================================
# Semaphore UI Automation Platform Container Configuration
...
variable "lxc_description" {
  ...
  type        = string
  default     = "Semaphore UI Automation Platform - Managed by OpenTofu"
}

variable "lxc_tags" {
  description = "Tags for container organization in Proxmox"
  type        = list(string)
  default     = ["lxc", "semaphore", "automation", "tofu-managed"]
}
...
# -----------------------------------------------------------------------------
# Semaphore-Specific Configuration
# -----------------------------------------------------------------------------

variable "lxc_semaphore_data_mount_volume" {
  description = "Host path for Semaphore data directory (bind mount)"
  type        = string
  default     = "/rpool/data/semaphore"
}

variable "lxc_semaphore_data_mount_path" {
  description = "Container path for Semaphore data"
  type        = string
  default     = "/var/lib/semaphore"
}

variable "service_user_uid" {
  description = "UID for semaphore service user (maps to 100900 on host for unprivileged containers)"
  type        = number
  default     = 900
...
variable "service_user_gid" {
  description = "GID for semaphore service group (maps to 100900 on host for unprivileged containers)"
  type        = number
  default     = 900
