#!/usr/bin/env bash
# =============================================================================
# Terraform/OpenTofu Functions for VM OpenSUSE Tumbleweed
# =============================================================================
...
terraform_init() {
    ...
    # Use -upgrade to ensure provider versions are consistent if needed, but not default
...
terraform_apply() {
    ...
    log_warning "This will create VM infrastructure on Proxmox"
    confirm "Proceed?" || { log_info "Cancelled"; return 1; }

    if ${iac_tool} apply -auto-approve tfplan 2>/dev/null || ${iac_tool} apply -auto-approve | tee -a "${LOG_FILE}"; then
...
terraform_destroy() {
    ...
    log_warning "This will DESTROY the VM and all its data!"
